...
function renderGamesGridInContainer(filteredGames = null, container) {
    ...
    container.innerHTML = gamesToRender.map((site, index) => {
        ...
        const description = site.description || 'Experience this exciting game with engaging gameplay and stunning graphics. Perfect for players looking for fun and challenge.';
    ...
    container.innerHTML = `
        <div class="slope-rider-center-card festive-game-cube game-cube">
            <div class="game-cube-inner">
                ...
                <div class="game-cube-description">${(slopeRider.description || '').substring(0, 100)}${(slopeRider.description || '').length > 100 ? '...' : ''}</div>
        ...
        <div class="festive-game-card">
            ...
            <div class="festive-game-content">
                ...
                <div class="festive-game-info">
                    ...
                    <button class="festive-play-btn" onclick="window.location.href='${getGamesPath(filename)}'">
            ...
            searchTimeout = setTimeout(() => {
                ...
                if (category !== 'all' && query === '' && !isAllGamesPage) {
                    if (category === 'popular') {
                        ...
                        // Show games that haven't been clicked yet OR were first clicked within the last 7 days
                        const sevenDaysAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
                        gamesToSearch = gamesToSearch.filter(game => {
                            return game.clicks === 0 || (game.firstClicked && game.firstClicked > sevenDaysAgo);
                        });
                ...
                const filtered = gamesToSearch
                    ...
                    .filter(site => {
                        ...
                        // Debug: log if searching for geometry dash related terms
                        if (query.toLowerCase().includes('ged') && site.title && site.title.toLowerCase().includes('geometry')) {
                            console.log('Search match check:', query, 'vs', site.title, '=', matches);
                        }
                        return matches;
                    })
...
function filterGamesByCategory(category) {
    ...
    if (category === 'popular') {
        ...
        // Show games that haven't been clicked yet (newly added) or recently added (within last 7 days)
        const sevenDaysAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
        filtered = filtered.filter(game => {
            // Include games with 0 clicks OR games that were first clicked within the last 7 days
            return game.clicks === 0 || (game.firstClicked && game.firstClicked > sevenDaysAgo);
        });
        // Sort by: games with 0 clicks first, then by firstClicked date (newest first)
        filtered.sort((a, b) => {
            // Prioritize games with 0 clicks
            if (a.clicks === 0 && b.clicks !== 0) return -1;
            if (a.clicks !== 0 && b.clicks === 0) return 1;
            // Then sort by firstClicked date (newer first)
            const firstA = a.firstClicked || 0;
            const firstB = b.firstClicked || 0;
    ...
    const CookieManager = {
        ...
        getPreferences: function() {
            return {
                ...
                analytics: this.getCookie(COOKIE_ANALYTICS_KEY) === 'true',
                advertising: this.getCookie(COOKIE_ADVERTISING_KEY) === 'true',
